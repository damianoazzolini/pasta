...
class TestEndlineContent(unittest.TestCase):
    def test_symbol_endline_or_space_n(self):
        self.assertEqual(pasta_parser.symbol_endline_or_space('\n'), True, "Endline content \\n not recognized")
    ...
    def test_symbol_endline_or_space_space(self):
        self.assertEqual(pasta_parser.symbol_endline_or_space(' '), True, "Endline content space not recognized")

    def test_symbol_endline_or_space_fail(self):
        self.assertEqual(pasta_parser.symbol_endline_or_space('a'), False, "a recognized as endline content")


class TestEndlineComment(unittest.TestCase):
    def test_symbol_endline_or_space_n(self):
        self.assertEqual(pasta_parser.endline_symbol('\n'), True, "Endline comment \\n not recognized")
    ...
    def test_symbol_endline_or_space_fail(self):
        self.assertEqual(pasta_parser.endline_symbol('a'), False, "a recognized as endline comment")


# commented since is_number is now in utils
...
class TestGetFunctor(unittest.TestCase):
    def test_atom(self):
        term = "a"
        self.assertEqual(pasta_parser.get_functor(term),term,term + "not recognized")

    def test_compound_arity_1(self):
        term = "a(f)"
        self.assertEqual(pasta_parser.get_functor(
...
class TestConsistencyProbFacts(unittest.TestCase):
    def test_correct_atom(self):
        ...
        fact = "0.5::a."
        self.assertEqual(pasta_parser.check_consistent_prob_fact(fact), (0.5, "a"), fact + " not recognized as probabilistic fact")

    def test_correct_term(self):
        # pars = pasta_parser.PastaParser("test.pl", 3)
        fact = "0.5::a(f)."
        self.assertEqual(pasta_parser.check_consistent_prob_fact(
    ...
    def test_wrong_prob(self):
        ...
        self.assertRaisesRegex(
            SystemExit, "Error: expected a float, found 0.5q", pasta_parser.check_consistent_prob_fact, fact)
    ...
    def test_not_valid_term_first_number(self):
        ...
        fact = "0.3::2l."
        self.assertRaisesRegex(
            SystemExit, "Invalid probabilistic fact 2l", pasta_parser.check_consistent_prob_fact, fact)

    def test_not_valid_term_first_upper(self):
        # pars = pasta_parser.PastaParser("test.pl", 3)
        fact = "0.3::Ul."
        self.assertRaisesRegex(
            SystemExit, "Invalid probabilistic fact Ul", pasta_parser.check_consistent_prob_fact, fact)

    def test_not_valid_term_symbol(self):
        # pars = pasta_parser.PastaParser("test.pl", 3)
        fact = "0.3::()."
        self.assertRaisesRegex(
            SystemExit, "Invalid probabilistic fact ()", pasta_parser.check_consistent_prob_fact, fact)
    ...
    def test_missing_end_dot(self):
        # pars = pasta_parser.PastaParser("test.pl", 3)
        fact = "0.5::a"
        self.assertRaisesRegex(SystemExit, "Missing final . in 0.5::a",
                               pasta_parser.check_consistent_prob_fact, fact)
