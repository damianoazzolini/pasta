...
def generate_generator(functor : str, args : str, arguments : list, prob : float, precision : int) -> Union[str,list]:
    ...
    if args is not None:
        ...
        generator = generator + vt + ":dom_" + functor + "}" + str(number) + "."
    ...
    if number != 1: # clauses for range
        ...
        if args is not None:
            ...
            show_declaration_t = "#show " + functor + "/" + str(args.count(',') + 2) + "."
            show_declaration_f = "#show not_" + functor + "/" + str(args.count(',') + 2) + "."
        else:
            clause_true = functor + "(" + str(prob_true) + ")" + ":-" + vt + "."
            clause_false = "not_" + functor + "(" + str(prob_false) + ")" + ":- not " + vt + "."
            auxiliary_clause_true = functor + ":- " + functor + "(_)."
            auxiliary_clause_false = "not_" + functor + ":- not_" + functor + "(_)."
